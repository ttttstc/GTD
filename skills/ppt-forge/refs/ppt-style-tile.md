...
| 维度 | 华为 | Apple | 阿里 | 通用 |
|------|------|-------|------|------|
| 主色 | 红/黑/灰 | 黑/白/渐变 | 橙/科技蓝 | 可配置 |
| 圆角 | 直角（0px） | 大圆角（12-16px） | 中圆角（8px） | 可配置 |
| 密度 | 高（模块化 Boxy） | 极低（每页一件事） | 中高（Dashboard） | 可配置 |
| 字重 | 粗体标语 + 细体数据 | 超大超粗标题 | 中等偏细 | 可配置 |
| 图标 | 面性/实心 | 线框/SF Symbols | Ant Design 线框 | 可配置 |
| 留白 | 最小化 | 最大化 | 适度 | 可配置 |
